...
def shift_mark_path():
    """Where marks are kept. RF_SHIFT_MARK points a test run somewhere harmless."""
    return os.environ.get("RF_SHIFT_MARK") or SHIFT_MARK_FILE


def _valid_window(w):
    return (isinstance(w, list) and len(w) == 2
            and (w[0] is None or isinstance(w[0], (int, float)))
...
def save_shift_marks(marks, today):
    """Write marks back, dropping days older than SHIFT_MARK_KEEP_DAYS. Returns the
    path written, or None -- like the station name, a mark is not worth failing a
    report over."""
    ...
    try:
        ...
        # Replace in one step, so a crash mid-write never leaves half a file behind.
        os.replace(tmp, path)
    except OSError:
        return None
    return path
